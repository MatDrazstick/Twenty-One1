...
import { AbilityCard, AbilityCategory } from './AbilityCard.js';
...
export class AIPlayer extends Player {
   ...
   */
  shouldHit(opponentVisibleScore: number, deckCardsRemaining: number): boolean {
   ...
   */
  private smartStrategy(myScore: number, opponentScore: number, cardsLeft: number): boolean {
   ...
   */
  private advancedStrategy(myVisibleScore: number, myTotalScore: number, opponentScore: number, cardsLeft: number): boolean {
   ...
   */
  chooseAbility(game: any, opponent: Player): number {
   ...
   */
  private basicAbilityStrategy(game: any, opponent: Player, myScore: number, opponentScore: number, target: number): number {
   ...
   */
  private conservativeAbilityStrategy(game: any, opponent: Player, myScore: number, opponentScore: number, target: number): number {
   ...
   */
  private smartAbilityStrategy(game: any, opponent: Player, myScore: number, opponentScore: number, target: number): number {
   ...
   */
  private advancedAbilityStrategy(game: any, opponent: Player, myScore: number, opponentScore: number, target: number): number {
    ...
    for (let i = 0; i < this.abilityHand.length; i++) {
      ...
      if (urgency === 'high') {
        if (ability.name === 'Bless') return i;
        if (ability.name === 'Shield-Plus') return i;
        if (ability.name === 'Shield') return i;
      }
      ...
      if (myScore > opponentScore) {
        if (ability.name === 'Disservice') return i;
        if (ability.name === 'Two-Up') return i;
        if (ability.name === 'Remove') return i;
      }
      
      // Comeback plays when losing
      if (opponentScore > myScore + 3) {
        if (ability.name === 'Perfect Draw' && needed > 0) return i;
        if (ability.name === 'Refresh') return i;
        if (ability.name === 'Exchange') return i;
      }
