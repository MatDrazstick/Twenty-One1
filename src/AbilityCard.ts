...
export enum AbilityCategory {
  AddNumber = 'AddNumber',
  DeckTrump = 'DeckTrump',
  BetAbility = 'BetAbility',
  GoFor = 'GoFor'
}

export interface AbilityEffect {
  (game: Game, player: Player, opponent: Player): boolean;
}
...
export class AbilityCard {
  ...
  value?: number; // For AddNumber abilities
...
export function createAllAbilityCards(): AbilityCard[] {
  const cards: AbilityCard[] = [];

  // === Add Number Abilities ===
  const addNumberValues = [2, 3, 4, 6, 7];
  for (const value of addNumberValues) {
    cards.push(new AbilityCard(
      `${value}-Card`,
      `This ability '${value}-Card' allows you to draw the numbered card ${value} if it's still available in the deck, if not the trump will simply be used but not draw the card.`,
      AbilityCategory.AddNumber,
      (game: Game, player: Player, opponent: Player) => {
  ...
  cards.push(new AbilityCard(
    'Hush',
    `'Hush' allows you to draw any card remaining from the deck however, it will be completely hidden what number you drew from your opponent.`,
    AbilityCategory.DeckTrump,
    (game: Game, player: Player, opponent: Player) => {
  ...
  cards.push(new AbilityCard(
    'Perfect Draw',
    `'Perfect Draw' allows you to draw the best remaining card remaining from the deck to reach the set number, it will however not work if there are no available cards from the deck or make you go over the set number.`,
    AbilityCategory.DeckTrump,
    (game: Game, player: Player, opponent: Player) => {
  ...
  cards.push(new AbilityCard(
    'Refresh',
    `'Refresh' allows you to draw 2 completely new cards remaining from the deck and return any of the cards you had in your deck before using this trump.`,
    AbilityCategory.DeckTrump,
    (game: Game, player: Player, opponent: Player) => {
  ...
  cards.push(new AbilityCard(
    'Remove',
    `'Remove' allows you remove the last card your opponent drew from the deck but this will not work if there is only one card remaining in the deck.`,
    AbilityCategory.DeckTrump,
    (game: Game, player: Player, opponent: Player) => {
  ...
  cards.push(new AbilityCard(
    'Return',
    `'Return' allows you withdraw the last card you drew from the deck.`,
    AbilityCategory.DeckTrump,
    (game: Game, player: Player, opponent: Player) => {
  ...
  cards.push(new AbilityCard(
    'Exchange',
    `'Exchange' allows you to swap the most recently taken card you have in your hand with your opponents.`,
    AbilityCategory.DeckTrump,
    (game: Game, player: Player, opponent: Player) => {
  ...
  cards.push(new AbilityCard(
    'Disservice',
    `'Disservice' forces your opponent to draw a card from the deck automatically and always works unless theres no more cards to draw.`,
    AbilityCategory.DeckTrump,
    (game: Game, player: Player, opponent: Player) => {
      if (game.deck.cardsRemaining() === 0) {
  ...
  cards.push(new AbilityCard(
    'One-Up',
    `'One-Up' allows you to increase the bet number by 1 therefore the counter will go up by 1 on the Kill Machine. However, if your opponent uses 'Destroy' or 'Reincarnation' the effect of the trump will be nullified and if someone uses another One-Up or Two-Up trump card when this is already in play the bet increase will stack.`,
    AbilityCategory.BetAbility,
    (game: Game, player: Player, opponent: Player) => {
  ...
  cards.push(new AbilityCard(
    'Two-Up',
    `'Two-up' allows you to increase the bet number by 2 therefore the counter will go up by 2 on the chainsaw. However, if your opponent uses 'Destroy' or 'Reincarnation' the effect of the trump will be nullified and if someone uses another One-Up or Two-Up trump card when this is already in play the bet increase will stack.`,
    AbilityCategory.BetAbility,
    (game: Game, player: Player, opponent: Player) => {
  ...
  cards.push(new AbilityCard(
    'Shield',
    `'Shield' allows you to decrease the bet number by 1 therefore the counter will go down by 1 on the Kill Machine. However, if your opponent uses 'Destroy' or 'Reincarnation' the effect of the trump will be nullified and if someone uses another Shield or Shield+ card when this is already in play the bet decrease will stack.`,
    AbilityCategory.BetAbility,
    (game: Game, player: Player, opponent: Player) => {
      game.betModifier = (game.betModifier || 0) - 1;
  ...
  cards.push(new AbilityCard(
    'Shield-Plus',
    `'Shield-Plus' allows you to decrease the bet number by 2 therefore the counter will go down by 2 on the Kill Machine. However, if your opponent uses 'Destroy' or 'Reincarnation' the effect of the trump will be nullified.`,
    AbilityCategory.BetAbility,
    (game: Game, player: Player, opponent: Player) => {
      game.betModifier = (game.betModifier || 0) - 2;
  ...
  cards.push(new AbilityCard(
    'Bless',
    `'Bless' allows you to avoid death even if you lose the round and the number on the tally is the same or higher then your counter - it's noted that this card only applies if you're about to die, if the tally is lower then your counter number and this card is used it will have no effect.`,
    AbilityCategory.BetAbility,
    (game: Game, player: Player, opponent: Player) => {
  ...
  cards.push(new AbilityCard(
    'Bloodfeast',
    `'Bloodfeast' allows you to draw one trump card and also increase the bet number by 1 on the tally. However, if your opponent uses 'Destroy' or 'Reincarnation' the effect of the trump will be nullified and if someone uses a 'Two-Up' or 'One-Up' trump it will stack the increased bet and vice versa if someone uses the 'Shield' or 'Shield+' trump it will decrease the bet. This can also be stacked with other Bloodshed trumps.`,
    AbilityCategory.BetAbility,
    (game: Game, player: Player, opponent: Player) => {
      game.betModifier = (game.betModifier || 0) + 1;
  ...
  cards.push(new AbilityCard(
    'Destroy',
    `'Destroy' allows you to destroy the most recently placed trump. However, if there are no trump cards placed on the table or the only placed trump cards were placed by you not your opponent then this trump will have no effect.`,
    AbilityCategory.BetAbility,
    (game: Game, player: Player, opponent: Player) => {
  ...
  cards.push(new AbilityCard(
    'Friendship',
    `'Friendship' allows you to draw two trump cards during the game. However, it also makes it so your opponent draws two trump cards as well when it's used.`,
    AbilityCategory.BetAbility,
    (game: Game, player: Player, opponent: Player) => {
      if (!game.abilityDeck) {
  ...
  cards.push(new AbilityCard(
    'Relentless',
    `'Relentless' allows you to destroy the most recently placed trump and draw one. However, if there are no trump cards placed on the table or the only placed trump cards were placed by you not your opponent then this trump will have no effect.`,
    AbilityCategory.BetAbility,
    (game: Game, player: Player, opponent: Player) => {
      if (game.lastAbilityPlayed && game.lastAbilityPlayed.player !== player) {
        const destroyed = game.lastAbilityPlayed.ability;
  ...
  for (const value of goForValues) {
    cards.push(new AbilityCard(
      `Go For ${value}`,
      `'Go For ${value}' allows you to change the number needed to reach from "21" to "${value}" therefore you'll need to draw or remove cards to reach ${value} instead of 21, if another 'Go For' card is applied or someone uses 'Destroy' or 'Reincarnation' this trumps effect will no longer apply.`,
      AbilityCategory.GoFor,
      (game: Game, player: Player, opponent: Player) => {
