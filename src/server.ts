...
function findRoomBySocket(socketId: string): RoomData | null {
...
function getPlayerIndex(room: RoomData, socketId: string): number {
...
io.on('connection', (socket: Socket) => {
  ...
  // Create new room
  socket.on('create-room', (data: { playerName: string; settings?: GameSettings }) => {
  ...
  // Player uses ability
  socket.on('use-ability', async (data: { abilityIndex: number }) => {
    const room = findRoomBySocket(socket.id);
    if (!room || !room.game) {
      socket.emit('error', { message: 'Not in a game' });
      return;
  ...
  // Reconnect to room
  socket.on('reconnect-room', (data: { roomCode: string; wasHost: boolean }) => {
    const roomCode = data.roomCode.toUpperCase();
    const room = rooms.get(roomCode);
...
function serializeGameState(game: Game, playerIndex: number) {
